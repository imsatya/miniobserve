...
> ⚠️ **Your graph nodes must call LangChain model wrappers (`ChatOpenAI`, `ChatAnthropic`, etc.), not the raw provider SDK (`openai.OpenAI()`, `anthropic.Anthropic()`).** Raw SDK calls bypass LangChain's callback system entirely — the handler registers correctly but never fires, and you will see `llm calls: 0` in the summary with no error. If you see `llm calls: 0` after a run where the callback was registered, set `MINIOBSERVE_TRACER_DIAG=1` — if the cause is a raw SDK call the diagnostic will confirm it.
